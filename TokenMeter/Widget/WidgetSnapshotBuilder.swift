...
enum WidgetSnapshotBuilder {
    ...
    private static func quotaOverlaySeries(
        ...
        let overlayWindowId = overlayWindowId(for: snapshots)

        let observations = snapshots
            .compactMap { snapshot -> QuotaWindowObservation? in
                guard let overlayWindowId,
                      let window = snapshot.windows.first(where: { $0.windowId == overlayWindowId })
                else {
                    return nil
                }
    ...
    /// Codex dropped the rolling 5h limit in July 2026; when the most recent
    /// snapshot no longer carries that window, overlay the weekly window instead.
    private static func overlayWindowId(for snapshots: [Track1Snapshot]) -> Track1WindowId? {
        guard let latest = snapshots.max(by: { $0.observedAt < $1.observedAt }) else {
            return nil
        }
        if latest.windows.contains(where: { $0.windowId == .rolling5h }) {
            return .rolling5h
        }
        if latest.windows.contains(where: { $0.windowId == .weekly }) {
            return .weekly
        }
        return nil
    }
