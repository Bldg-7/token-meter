...
enum CodexTrack1MethodBAdapterError: Error, Equatable {
    case missingWindows
    case invalidWindow(index: Int)
    case invalidJSON
}

struct CodexTrack1MethodBAdapter {
    static let parserVersion = "codex_track1_method_b_v1"
    ...
    static func snapshot(from data: Data, observedAt: Date = Date()) throws -> Track1Snapshot {
        ...
        do {
            ...
            throw CodexTrack1MethodBAdapterError.invalidJSON
        }

        guard payload.windows.isEmpty == false else {
            throw CodexTrack1MethodBAdapterError.missingWindows
        }
        ...
        for (idx, w) in payload.windows.enumerated() {
            guard let windowId = Track1WindowId.normalized(from: w.windowId) else {
                throw CodexTrack1MethodBAdapterError.invalidWindow(index: idx)
            }
