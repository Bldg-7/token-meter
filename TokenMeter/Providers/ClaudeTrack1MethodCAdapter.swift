...
enum ClaudeTrack1MethodCAdapterError: Error, Equatable {
    case missingWindows
    case invalidWindow(index: Int)
    case invalidJSON
}

struct ClaudeTrack1MethodCAdapter {
    static let parserVersion = "claude_track1_method_c_v1"

    static func snapshot(from data: Data, observedAt: Date = Date()) throws -> Track1Snapshot {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601

        let payload: MethodCPayload
        do {
            payload = try decoder.decode(MethodCPayload.self, from: data)
        } catch {
            throw ClaudeTrack1MethodCAdapterError.invalidJSON
        }

        guard payload.windows.isEmpty == false else {
            throw ClaudeTrack1MethodCAdapterError.missingWindows
        }
        ...
        for (idx, w) in payload.windows.enumerated() {
            guard let windowId = Track1WindowId.normalized(from: w.windowId) else {
                throw ClaudeTrack1MethodCAdapterError.invalidWindow(index: idx)
            }
...
private struct MethodCPayload: Decodable {
    var plan: String?
    var windows: [MethodCWindow]
    ...
    init(from decoder: Decoder) throws {
        ...
        if let windows = try c.decodeIfPresent([MethodCWindow].self, forKey: .windows) {
            self.windows = windows
        } else if let limits = try c.decodeIfPresent([MethodCWindow].self, forKey: .limits) {
            self.windows = limits
        } else if let usage = try c.decodeIfPresent(MethodCContainer.self, forKey: .usage) {
            self.windows = usage.windows
        } else if let data = try c.decodeIfPresent(MethodCContainer.self, forKey: .data) {
            self.windows = data.windows
        } else if let result = try c.decodeIfPresent(MethodCContainer.self, forKey: .result) {
...
private struct MethodCContainer: Decodable {
    var windows: [MethodCWindow]
    ...
    init(from decoder: Decoder) throws {
        ...
        if let windows = try c.decodeIfPresent([MethodCWindow].self, forKey: .windows) {
            ...
            self.windows = limits
        } else if let items = try c.decodeIfPresent([MethodCWindow].self, forKey: .items) {
            self.windows = items
        } else {
            self.windows = []
        }
...
private struct MethodCWindow: Decodable {
    var windowId: String
    ...
    init(from decoder: Decoder) throws {
        ...
        self.usedPercent = try decodeFirst(Double.self, keys: [.usedPercent, .used_percent, .usedPct, .used_pct, .used])
        self.remainingPercent = try decodeFirst(Double.self, keys: [.remainingPercent, .remaining_percent, .remainingPct, .remaining_pct, .remaining])
